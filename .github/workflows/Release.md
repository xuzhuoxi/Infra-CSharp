...
可调整常量在 `Release.yml` 顶部的 `env`：`APP_NAME`（DLL 与 zip 名）、`TARGET_TFM`（目标框架）。
...
可参考模板 `notes/release/ReleaseNotes.md`。该模板本身 **不会** 被 workflow 读取。也可用仓库内 Cursor Skill `generate-note` 根据提交范围生成该文件。
...
| `.cursor/skills/generate-note/` | 根据提交范围生成说明文件 |
